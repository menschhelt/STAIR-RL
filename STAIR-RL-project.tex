...
CQL-SAC로 18개월 과거 데이터에서 안전한 초기 정책을 학습한 후, PPO-CVaR로 18개월 검증 데이터에서 최근 시장에 적응시킨다.
...
M2 통화량과 WALCL(연준 대차대조표)은 유동성 상황을 반영하여 암호화폐 수요를 예측하는 데 활용되며,
Fama-French 팩터(SMB, HML)는 대형주 대비 소형주 선호도 변화를 포착한다.
...
본 연구의 투자 대상은 일별 Quote Volume 기준 상위 20개 암호화폐로 구성된다.
선정 기준은 24시간 Quote Volume(USDT 기준)이며, USDT, USDC, BUSD 등 스테이블코인은 투자 대상에서 제외된다.
...
\subsubsection{학습/검증/테스트 기간}

Temporal leakage를 방지하기 위해 데이터셋을 시간적으로 엄격히 분리하였다.
훈련 데이터는 2021년 1월부터 2022년 6월까지 18개월 동안 Phase 1 CQL-SAC 학습에 사용되었다.
이후 6개월의 시간적 갭(2022년 7월--12월)을 두어 과적합을 방지하고,
검증 데이터는 2023년 1월부터 2024년 6월까지 18개월 동안 Phase 2 PPO-CVaR 미세조정에 활용되었다.
...
글로벌 특징은 $(T=5, 28)$로 23개 거시경제 지표와 5개 Fama-French 팩터로 구성된다.
...
% TensorBoard 스크린샷 삽입 위치
\begin{figure}[h]
\centering
% \includegraphics[width=\textwidth]{figures/phase1_tensorboard.png}
\fbox{\parbox{0.9\textwidth}{\centering\vspace{3cm}\textit{[Phase 1 TensorBoard 스크린샷 삽입 예정]}\vspace{3cm}}}
\caption{Phase 1 (CQL-SAC) 학습 곡선. 상단: Batch Reward (안정), 하단: Loss 곡선들 (Actor plateau 도달, CQL 안정)}
...
Phase 2는 2023.01-2024.06 검증 데이터에서 온라인 환경과 상호작용하며 CVaR 제약 하에서 정책을 최적화한다.
...
\begin{figure}[h]
\centering
% \includegraphics[width=\textwidth]{figures/phase2_initial_tensorboard.png}
\fbox{\parbox{0.9\textwidth}{\centering\vspace{2.5cm}\textit{[Phase 2 초기 학습 TensorBoard: Lambda 증가, CVaR 미수렴 패턴]}\vspace{2.5cm}}}
\caption{Phase 2 초기 학습 결과 (157k steps). Lambda가 단조 증가하고 CVaR이 개선되지 않는 문제가 관찰되어 하이퍼파라미터 조정 후 재시작하였다.}
...
\begin{figure}[h]
\centering
% \includegraphics[width=\textwidth]{figures/phase2_adjusted_tensorboard.png}
\fbox{\parbox{0.9\textwidth}{\centering\vspace{2.5cm}\textit{[Phase 2 조정 후 학습 TensorBoard 삽입 예정]}\vspace{2.5cm}}}
\caption{Phase 2 하이퍼파라미터 조정 후 학습 곡선. $\lambda_{\max}=5.0$, $\kappa=20\%$ 적용.}
...
% TODO: Phase 2 완료 후 테스트셋 결과 추가
% - 테스트셋(2025.01-2025.11) 성과 지표
% - 연간 수익률, Sharpe ratio, CVaR95, Maximum Drawdown, Calmar ratio

\paragraph{PPO-Only 베이스라인}
전이학습의 실효성을 검증하기 위해, CQL-SAC 사전학습 없이 PPO-CVaR만으로 처음부터 학습하는 베이스라인 실험을 수행하였다.
공정한 비교를 위해, 이 베이스라인은 Phase 2와 \textbf{동일한 기간}(2023년 1월$\sim$2024년 6월, 18개월)의 데이터를 사용한다.
유일한 차이점은 초기 가중치가 무작위인지(PPO-only) 사전학습된 것인지(전이학습)이다.
이를 통해 전이학습의 효과를 정확히 측정할 수 있다.
PPO-only는 높은 학습률($3 \times 10^{-4}$)로 학습을 시작하며, on-policy 알고리즘 특성상 현재 정책이 생성하는 데이터에서 직접 학습한다.
다만, 랜덤 정책에서 시작하는 탐험 비용이 발생할 수 있으며, 이는 2단계 접근법이 해결하고자 했던 cold-start 문제이다.

% TODO: PPO-Only 학습 완료 후 결과 추가
% - 학습 곡선 비교 (전이학습 vs PPO-only)
% - 테스트셋 성과 비교
% - CVaR 제약 충족 여부 비교

\paragraph{정량적 성과 (예정)}
두 접근법 모두 테스트셋(2025.01-2025.11)에서 평가될 예정이다.
연간 수익률, Sharpe ratio, CVaR$_{95}$, Maximum Drawdown, Calmar ratio를 벤치마크(동일 비중 포트폴리오, 시가총액 가중 포트폴리오)와 비교 분석한다.
특히 CVaR 제약($\kappa = 10\%$)이 실제로 유지되었는지, 그리고 리스크 조정 수익률(Sharpe, Sortino)에서 벤치마크 대비 개선이 있었는지를 평가한다.
...
본 프로젝트의 핵심 이론적 기여는 semantic information의 value를 information-theoretically 정량화한 것이다.
정리 1은 semantic token이 제공하는 value improvement가 conditional mutual information의 $O(\sqrt{I})$로 \textbf{upper bound}임을 보였다.
이 결과는 Pinsker inequality와 value function의 Lipschitz continuity에서 유도되며, information-theoretic constraint를 반영한다.
$\sqrt{I}$ 스케일링은 정보량이 증가해도 value improvement가 무한히 증가하지 않으며, 수확 체감(diminishing returns)이 발생함을 의미한다.
이는 TERC가 왜 필요한지---모든 토큰을 사용하는 것보다 핵심 토큰만 선별하는 것이 효율적인지---에 대한 이론적 근거를 제공한다.

정리 2의 transfer stability bound는 offline 학습에서 online 적용으로의 transfer 과정에서 CVaR 제약이 유지됨을 보장한다.
CQL의 conservative Q-function estimation과 PPO의 clipping이 결합되어, policy shift가 제한된 범위 내에서 이루어진다.

\subsection{구현 성과}

기술적으로는 네 가지 주요 모듈을 구현했다.
TERC 기반 token selection mechanism은 transfer entropy 계산과 학습 가능한 importance MLP를 hybrid로 결합했다.
Dynamic semantic gating은 numerical market state에 따라 semantic information의 가중치를 자동 조절한다.
Late Fusion architecture는 Alpha factor, OHLCV, news, social media의 네 가지 modality를 효과적으로 결합하며, $z_{\text{pooled}}$와 $z_{\text{unpooled}}$의 dual output으로 Critic과 Actor에 각각 적합한 representation을 제공한다.
Two-stage transfer learning pipeline은 CQL-SAC offline pre-training에서 PPO-CVaR online fine-tuning으로 연결된다.

본 프로젝트의 모든 구현 코드는 GitHub 저장소에서 확인할 수 있다.
...
현재 구현에는 몇 가지 한계가 있다.
Shapley-Gate Alignment와 Semantic Smoothing 손실은 계산 비용 문제로 비활성화($\lambda=0$)되어 있으며, 이러한 제약이 실험 성능에 영향을 미쳤을 가능성이 있다.

향후 연구에서는 FastSHAP 같은 계산 효율적 Shapley 근사 방법을 적용하고, 주식이나 외환 같은 다른 자산군에 대한 일반화를 검증할 필요가 있다.

\textbf{Two-stage transfer learning의 한계}: 정리 2의 transfer stability bound는 source domain과 target domain 간의 분포 변화가 bounded할 때만 유효하다.
그러나 본 실험에서 Phase 1 학습 기간(2021-2022, 상승장 후 급락)과 Phase 2 적용 기간(2023-2024, 회복기)은 시장 레짐이 근본적으로 달랐다.
이로 인해 Phase 1에서 학습된 인코더와 value function이 Phase 2에서 오히려 해로운 편향(harmful bias)을 전이했을 가능성이 있다.
On-policy 알고리즘인 PPO만을 사용하여 현재 데이터에서 처음부터 학습했다면, 과거 레짐의 편향 없이 더 나은 적응이 가능했을 수 있다.
이를 검증하기 위해 PPO-only baseline과의 비교 실험이 필요하며, 레짐 변화가 심한 금융 시장에서 offline-to-online transfer의 유효성에 대한 추가 연구가 요구된다.
